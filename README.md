...
最新版を GitHub Releases からインストールする．
...
```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/knrew/dotkoke/releases/latest/download/install.sh | sh -s -- --to "$HOME/.local/bin"
```

target を明示する場合:

```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/knrew/dotkoke/releases/latest/download/install.sh | sh -s -- --target x86_64-unknown-linux-musl
```

特定バージョンをインストールする場合:

```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/knrew/dotkoke/releases/download/v0.1.0/install.sh | sh
```

`install.sh` は対応 platform を自動判定し，release archive を download して checksum を検証した上で `dotkoke` をインストールする．
default のインストール先は `$HOME/.local/bin` である．このディレクトリが `PATH` に含まれていない場合は，利用者側で `PATH` に追加する必要がある．

### Cargo でのインストール

Rust toolchain がある場合は，tagged release を指定してインストールできる．

```sh
cargo install --locked --git https://github.com/knrew/dotkoke --tag v0.1.0 dotkoke
```

### 開発版

default branch の最新状態をインストールする場合:

```sh
cargo install --locked --git https://github.com/knrew/dotkoke dotkoke
```

これは未リリースの変更を含む可能性があり，安定版としては扱わない．
正式なインストール方法は，GitHub Releases の `install.sh` と tagged release を指定した `cargo install` である．
...
`$HOME` 側の対応パスがリンク先を解決できない壊れたシンボリックリンクの場合は削除する．
存在しないパスを指定した場合はエラーにする．
